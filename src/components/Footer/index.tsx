    ...
    <footer className="bg-slate-900 text-slate-300">
      ...
      <div className="container mx-auto grid gap-10 px-6 py-12 md:grid-cols-3">
            ...
            {SOCIALS.map((social) => {
              ...
              return (
                <a
                  ...
                  className={`text-slate-400 transition-colors hover:text-blue-400 ${FOCUS_RING}`}
                >
                  <Icon />
                </a>
              );
        ...
        <nav aria-label="Footer">
          <h3 className="text-sm font-bold uppercase tracking-widest text-slate-500">Navigate</h3>
          <ul className="mt-4 grid grid-cols-2 gap-x-8 gap-y-3 text-sm">
            {NAV_LINKS.map((link) => (
              <li key={link.id}>
                <a
                  href={`#${link.id}`}
                  className={`group inline-flex items-center gap-1.5 transition-colors hover:text-blue-400 ${FOCUS_RING}`}
                >
                  <span
      ...
      <div className="border-t border-slate-700/60">
        <div className="container mx-auto flex flex-col items-center justify-between gap-3 px-6 py-5 text-xs text-slate-500 sm:flex-row">
          <p>© 2026 Alejandro Román. All rights reserved.</p>
          <p className="flex items-center gap-2.5">
            Built with
            <span className="flex items-center gap-2.5 text-base text-slate-400">
          ...
          <a
            href="#Home"
            className={`inline-flex items-center gap-1.5 font-semibold text-slate-400 transition-colors hover:text-blue-400 ${FOCUS_RING}`}
          >
            Back to top
